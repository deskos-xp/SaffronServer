...
from .UpdateBrand import UpdateBrand
import string
class NewBrandDialogController(QDialog):
    brand_address_add="{address}/brand/update/{id}/add/address/{address_id}"
    auth:tuple=None
    #address:str=None
    w=None
    widget=None
    ...
    def __init__(self,widget,address,auth):
        self.widget=widget
        super(NewBrandDialogController,self).__init__()
        ...
        widget.setWindowTitle("New Brand")
        #QDialog.setW
    ...
    def confirmed(self):
        ...
        #json is now built, send to server to get brand id
        self.brand=NewBrand()
        self.brand.w=self.w
        self.brand.widget=self.widget
        self.brand.auth=self.w.auth
        self.brand.address=self.w.address
        self.brand.json=tmp_json
        self.brand.brandSent.connect(self.setBrandID)
        self.brand.start()
        ...
        #send address update for brand to server
    ...
    def setBrandID(self,ID:int): 
            tmp_address=self.widget.address.currentText().split(":")[0]
            addr_str=self.brand_address_add.format(**dict(address=self.address,id=ID,address_id=tmp_address))

...
from .NewVendor import NewVendor
from .UpdateVendor import UpdateVendor
import string
class NewVendorDialogController(QDialog):
    vendor_address_add="{address}/vendor/update/{id}/add/address/{address_id}"
    auth:tuple=None
    #address:str=None
    w=None
    widget=None
    ...
    def __init__(self,widget,address,auth):
        self.widget=widget
        super(NewVendorDialogController,self).__init__()
        ...
        widget.setWindowTitle("New Vendor")
    ...
    def confirmed(self):
        ...
        #json is now built, send to server to get vendor id
        self.vendor=NewVendor()
        self.vendor.w=self.w
        self.vendor.widget=self.widget
        self.vendor.auth=self.w.auth
        self.vendor.address=self.w.address
        self.vendor.json=tmp_json
        self.vendor.vendorSent.connect(self.setVendorID)
        self.vendor.start()
        ...
        #send address update for vendor to server
    ...
    def setVendorID(self,ID:int): 
            ...
            addr_str=self.vendor_address_add.format(**dict(address=self.address,id=ID,address_id=tmp_address))

...
class vendorThread(QThread,ComboBox_Update):
    time=0.5
    w=None
    auth=None
    address=None
    ...
    def vendor_setup(self):
        try:
            cb=self.w.vendor_cb
            if self.address == None:
                return
            status_response=requests.post("{}/vendor/get".format(self.address),auth=self.auth,json=dict(page=0,limit=sys.maxsize))
            if status_response.status_code == 200:
                ...
                if 'objects' in status.keys():
                    ...
                    cb.activated[str].connect(self.vendor_cb_onChange)
        except Exception as e:
            self.w.root.statusBar().showMessage(str(e))
            time.sleep(2)
            self.w.root.statusBar().clearMessage()
    def vendor_cb_onChange(self,text):
        cb=self.sender()
        print(text)
